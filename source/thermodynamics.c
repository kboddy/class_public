...
int thermodynamics_at_z(
  ...
  if (z >= pth->z_table[pth->tt_size-1]) {
    ...
    pvecthermo[pth->index_th_dTb] = -pba->T_cmb*pvecback[pba->index_bg_H];
...
int thermodynamics_onthespot_energy_injection(
  ...
  rho_cdm_today = pow(pba->H0*_c_/_Mpc_over_m_,2)*3/8./_PI_/_G_*(pba->Omega0_idm_dr+pba->Omega0_cdm+pba->Omega0_dmeff)*_c_*_c_; /* energy density in J/m^3 */
...
int thermodynamics_energy_injection(
  ...
  if (preco->annihilation > 0) {
    ...
    if (preco->has_on_the_spot == _FALSE_) {
      ...
      factor = _sigma_ * nH0 / pba->H0 * _Mpc_over_m_ / sqrt(pba->Omega0_b+pba->Omega0_cdm+pba->Omega0_dmeff+pba->Omega0_idm_dr);
...
int thermodynamics_recombination_with_hyrec(
  ...
  param.omh2 = (pba->Omega0_b+pba->Omega0_cdm+pba->Omega0_idm_dr+pba->Omega0_ncdm_tot+pba->Omega0_dmeff)*pba->h*pba->h;
...
int thermodynamics_output_titles(struct background * pba,
  ...
  //class_store_columntitle(titles,"Tb'",_TRUE_);
